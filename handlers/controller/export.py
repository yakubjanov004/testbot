...
def get_controller_export_router():
    ...
    async def export_menu_handler(message: Message, state: FSMContext):
        ...
        try:
            ...
            print(f"Error in controller export handler: {str(e)}")
            ...
            for fmt in formats:
                keyboard.append([
                    InlineKeyboardButton(
                        ...
                        callback_data=f"controller_format_{fmt}"
                    )
                ])
    ...
    async def handle_export_format(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            try:
                # Create export file
                file_content, filename = create_export_file(export_type, format_type, "controller")
