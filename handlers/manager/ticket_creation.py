"""
Manager Ticket Creation Handler

This module handles ticket creation for managers using shared ticket creation.
"""
...
def get_manager_ticket_creation_router():
    """Get manager ticket creation router"""
    ...
    async def start_connection_order(message: Message, state: FSMContext):
        """Start connection order creation for manager"""
        try:
            await start_ticket_creation(message, state, "manager")
    ...
    async def start_technical_service(message: Message, state: FSMContext):
        """Start technical service creation for manager"""
        try:
            await start_ticket_creation(message, state, "manager")
        except Exception as e:
