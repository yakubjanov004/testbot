...
from keyboards.controller_new_buttons import get_back_keyboard, get_priority_keyboard, get_technician_assignment_keyboard
from utils.language import get_text
from utils.database import get_user_language, create_technical_service, get_available_technicians
...
async def handle_create_technical(message: Message, state: FSMContext):
    """Texnik xizmat yaratish handler for controller"""
...
async def handle_description_input(message: Message, state: FSMContext):
    ...
    from keyboards.controller_new_buttons import InlineKeyboardBuilder, InlineKeyboardButton
...
async def handle_priority_selection(callback: CallbackQuery, state: FSMContext):
    ...
    if not technicians:
        text = get_text("no_technicians_available", lang)
        await callback.message.edit_text(text, reply_markup=get_back_keyboard(lang))
        await callback.answer()
        return
    
    # Show technician assignment options
    text = get_text("select_technician", lang)
    keyboard = get_technician_assignment_keyboard(technicians, 0, lang)  # 0 as placeholder application_id
    
    await callback.message.edit_text(text, reply_markup=keyboard)
    await state.set_state(TechnicalServiceStates.waiting_for_technician)
    await callback.answer()

@router.callback_query(F.data.startswith("assign_tech_"))
async def handle_technician_assignment(callback: CallbackQuery, state: FSMContext):
    """Handle technician assignment for technical service"""
    lang = await get_user_language(callback.from_user.id)
    technician_id = int(callback.data.split("_")[2])
    
    await state.update_data(technician_id=technician_id)
    ...
    from keyboards.controller_new_buttons import get_confirmation_keyboard
