...
from utils.database import get_user_language, create_connection_application, get_available_technicians
...
async def handle_create_connection(message: Message, state: FSMContext):
    """Ulanish arizasi yaratish handler for controller"""
...
async def handle_priority_selection(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(ConnectionApplicationStates.waiting_for_technician)
    await callback.answer()
...
async def handle_technician_assignment(callback: CallbackQuery, state: FSMContext):
    """Handle technician assignment for connection application"""
    lang = await get_user_language(callback.from_user.id)
